...
export const zhHant = defineLocale({
  ...
  sidebar: {
    ...
    allInFolders: '每個對話都已歸入專案或群組。',
    groups: {
      sectionLabel: '群組',
      newButton: '新增群組',
      createTitle: '新增群組',
      createDesc: '群組只是一個名稱，用來整理對話，不會移動任何檔案。',
      renameTitle: '重新命名群組',
      namePlaceholder: '例如：週五發布',
      empty: '還沒有群組。群組用來整理對話，不會移動檔案。',
      menu: '動作',
      rename: '重新命名…',
      delete: '刪除群組',
      deleteConfirm: (name: string) => `刪除群組「${name}」？其中的對話會維持原狀。`,
      deleteFailed: '無法刪除群組',
      reorderFailed: '無法重新排序群組',
      filedIn: (name: string) => `已歸入 ${name}`
    },
    projects: {
      empty: '還沒有專案。專案就是 Hermes 工作的資料夾。',
  ...
  chat: {
    context: {
      ...
      folderTitle: '資料夾',
      noFolder: '無資料夾',
      inGroup: name => `位於 ${name}`,
      addToGroup: '加入群組',
      branchTitle: '分支',
      branchSwitched: branch => `已切換至 ${branch}`,
      branchFailed: '無法切換分支',
      changeProject: '改在其他專案中作業…',
      chooseFolder: '選擇資料夾…',
      moveTitle: '移動此對話?',
      moveConfirm: place => `此對話將改在 ${place} 中作業。終端機與檔案工具也會隨之變更,之後的操作都在那裡執行。`,
      moveConfirmLabel: '移動',
      moved: place => `現在在 ${place} 中作業`,
      moveFailed: '無法移動此對話'
    }
  },
